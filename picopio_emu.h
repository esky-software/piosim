...
//fprintf(stderr, "pop %d\n", b1);
		...
		if (_pio_info_g.cycles > _pio_info_g.csvin_info.cycles) {	// (past)
			...
			_pio_info_g.csvin_cache_valid = false;
			if (_pio_info_g.ftrace)
				fprintf(_pio_info_g.ftrace, "\tREAD CSV (cycle %d)\n", _pio_info_g.csvin_info.cycles);
		}
...
static void pio_exec_instruction(void)
{
	...
	if (_pio_info_g.delay_timer > 0) {
		...
		if (_pio_info_g.ftrace)
			fprintf(_pio_info_g.ftrace, "cycle %d\t\t\tDELAY\n", _pio_info_g.cycles);
	...
	if (_pio_info_g.osr_autopull == true && _pio_info_g.osr_bitctr >= _pio_info_g.osr_autopull_threshold) {	// reached to autopull threshold
		...
		if (pio_pop_txfifo(&pop_val) == true) {
			...
			if (_pio_info_g.ftrace)
				fprintf(_pio_info_g.ftrace, "\tAUTOPULL\n");
			autopull_done		= true;
		}
		else {
...
void pio_run_emulation(int cycles, FILE *file_in, FILE *file_out, FILE *file_trace, FILE *file_asm)
{
	pio_begin_emulation(file_in, file_out, file_trace, file_asm);
	pio_step_emulation(cycles);
	pio_end_emulation();
}

void pio_begin_emulation(FILE *file_in, FILE *file_out, FILE *file_trace, FILE *file_asm)
{
	...
	_pio_info_g.fin = file_in;
	_pio_info_g.fout = file_out;
	_pio_info_g.ftrace = file_trace;
	_pio_info_g.fasm = file_asm;
...
void pio_step_emulation(int cycles)
{
	int ctr;
